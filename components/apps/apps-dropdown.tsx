    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="sm"
          className="relative h-9 px-3 gap-2 rounded-md text-sm hover:bg-secondary transition-colors">
          <Zap className="h-4 w-4" />
          Apps
          {connectedApps.length > 0 && (
            <span className="absolute -top-1 -right-1 h-4 min-w-[16px] flex items-center justify-center bg-primary text-[10px] font-medium text-primary-foreground px-1 rounded-full border-2 border-background">
      ...
      <PopoverContent align="end" sideOffset={8}
        className="w-72 p-0 rounded-lg border shadow-lg overflow-hidden bg-popover">
        ...
        <div className="p-3 space-y-4 max-h-[400px] overflow-y-auto">
          ...
          {connectedApps.length > 0 && (
            <div>
              <p className="text-xs font-medium text-emerald-600 px-1 mb-2">Connected Apps</p>
              <div className="space-y-1">
                {connectedApps.map(app => {
                  const Logo = app.Logo;
                  return (
                    <Link key={app.id} href="/apps" onClick={() => setIsOpen(false)}>
                      <div className="flex items-center gap-3 p-2.5 rounded-md hover:bg-muted transition-colors group cursor-pointer">
                        <div className={cn("h-8 w-8 rounded-md flex items-center justify-center flex-shrink-0", app.bg)}>
                          <Logo size={18} />
                        </div>
              ...
              <div className="space-y-1">
                {suggestedApps.map(app => {
                  ...
                  return (
                    <Link key={app.id} href="/apps" onClick={() => setIsOpen(false)}>
                      <div className="flex items-center gap-3 p-2.5 rounded-md hover:bg-muted transition-colors group cursor-pointer">
                        <div className={cn("h-8 w-8 rounded-md flex items-center justify-center flex-shrink-0 opacity-70", app.bg)}>
                          <Logo size={18} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium truncate">{app.name}</p>
        ...
        <div className="p-3 border-t">
          <Link href="/apps" onClick={() => setIsOpen(false)}>
            <Button className="w-full rounded-md text-sm bg-primary hover:bg-primary/90">
