...
function detectAction(prompt: string): { action: string; confidence: number; params: Record<string, string | undefined> } | null {
  ...
  const patterns: Array<{ action: string; keywords: string[]; confidence: number; extract: (input: string) => Record<string, string | undefined> }> = [
    { action: "create_task", keywords: ["create", "task"], confidence: 0.95, extract: (i) => {
        ...
        // Try quoted title first, then unquoted (with priority/due/assign as boundaries)
        const titleMatch = i.match(/create\s+(?:a\s+)?task\s+(?:called|named|titled|title)?\s*["']([^"']+)["']/i)
          || i.match(/create\s+(?:a\s+)?task\s+(?:called|named|titled|title)?\s*([^"']+?)\s+(?:with|having|priority|due|assign|status|$)/i)
          || i.match(/create\s+(?:a\s+)?task\s+(?:called|named|titled|title)?\s*(["']?[^"']+["']?)$/i);
        params.title = titleMatch ? titleMatch[1].replace(/["']/g, "").trim() : "New task";
        const priorityMatch = i.match(/\b(?:priority|priority\s+of)\s+(low|medium|high|urgent)\b/i)
          || i.match(/\b(low|medium|high|urgent)\s+priority\b/i);
        if (priorityMatch) params.priority = priorityMatch[1].toLowerCase();
        const statusMatch = i.match(/\bstatus\s+(?:of\s+)?(\w+)/i);
        if (statusMatch) params.status = statusMatch[1];
        const dueMatch = i.match(/\bdue\s+(?:date\s+)?(\d{4}-\d{2}-\d{2}|tomorrow|next\s+\w+)/i);
    ...
    { action: "create_project", keywords: ["create", "project"], confidence: 0.95, extract: (i) => {
        const m = i.match(/create\s+(?:a\s+)?project\s+(?:called|named)?\s*["']([^"']+)["']/i)
          || i.match(/create\s+(?:a\s+)?project\s+(?:called|named)?\s*([^"']+?)$/i);
        return m ? { name: m[1].replace(/["']/g, "").trim() } : { name: "New project" };
    } },
