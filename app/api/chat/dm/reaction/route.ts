...
export async function POST(req: Request) {
  try {
    ...
    const message = await prisma.chatMessage.findUnique({
      where: { id: messageId },
      select: { id: true, reactions: true, conversationId: true, workspaceId: true },
    });
    ...
    if (!message.conversationId) {
      return NextResponse.json({ error: "Not a DM message" }, { status: 400 });
    }
    ...
    publishToChannel(`dm:${message.conversationId}`, "message:updated", {
      id: messageId,
      reactions,
    });
    ...
    console.error("[DM REACTION_ERROR]", error);
